...
module.updateAllStations = function(cb, values) {
    var async = require('async')
    var stationsFunctions = []

	module.getCaptcha(module.entities.cities[5], function(err, res) {
		if(err) cb(err)
		...
		async.parallelLimit(stationsFunctions, 10, function (error, response) {
...
module.getStationsByCity = function(city, fuelType, values, cb) {
    ...
    var scratchStations = function(err, res) {
        ...
        console.log('stations from 0 completed, fuel 1'
        				.replace('0', city.name)
    ...
    module.post(
        'http://www.anp.gov.br/preco/prc/Resumo_Por_Municipio_Posto.asp', {
            Tipo: values.tipo, //i don't know what is it
            selSemana: values.selSemana, //not required/verified
            cod_Semana: values.codSemana,
            selCombustivel: module.anpEntityToStr(fuelType),
            selMunicipio: module.anpEntityToStr(city),
            txtValor: values.captcha, //catcha
        },
        scratchStations
    )
...
module.exports.postProcessStations = function(cb) {
	...
	var putFuelPrice = function (station) {
		var fuelType = module.entities.fuelTypes.filter(function(f) { return f.id == station.fuelTypeId })[0]
		station[fuelType.name + 'PrecoVenda'] = station.precoVenda
		station[fuelType.name + 'PrecoCompra'] = station.precoCompra
		station[fuelType.name + 'Modalidadecompra'] = station.modalidadecompra
		station[fuelType.name + 'fornecedorBBranca'] = station.fornecedorBBranca
	...
	module.entities.cities.forEach(function(city) {
		...
		if(city.stations.length > 0) {
			city.stations.forEach(function(s) { putFuelPrice(s) })
