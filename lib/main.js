...
if((hasArgument('shutdown') || hasArgument('-sh')) && isRoot) {
	console.log('The pc will shutdown when process end')
} else if (hasArgument('shutdown') || hasArgument('-sh')) {
	...
	updateStations.anpUpdate(true, function(err, res) {
		...
		if(shutdown && isRoot) {
			...
			if(duration > 5) { // more than 5 minutes, than shutdown
				try {
					require('fs').writeFileSync('anp-update-log.log', 'date: {0}, started: {1}, end: {2}'
							    .replace('{0}', new Date().toString())
							    .replace('{1}', startTime.toJSON().split('T')[1])
							    .replace('{2}', new Date().toJSON().split('T')[1]))
