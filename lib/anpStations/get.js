...
var fs = require('fs')
var req = require(path.join(rootPath, 'request.js'))
var strOp = require(path.join(rootPath, 'stringOperations.js'))
var anpEntitities = require(path.join(rootPath, 'getStations', 'anpEntitities.js'))
...
module.exports.test = function (cb) {
	...
	var city = anpEntities.cities.filter(function(c) { return c.name.toLowerCase().indexOf('campo largo') > -1 })[0]

	module.getCaptcha(city, new Date,
			  function(err, values) {
				  if(err) console.log(err)

					  anpEntities.fuelTypes.forEach(function(f) {
...
module.exports.anpUpdated = function (cb) {
	...
	var lastUpdatedDate = anpEntities.stations.reduce(reduceByDate).dataColeta
	lastUpdatedDate = strOp.dateStringToDate(lastUpdatedDate)

	//verify just the most popular fuel: gas
	var fuelType = anpEntities.fuelTypes.filter(function(f) { return f.name.toLowerCase().indexOf('gasolina') > -1 })[0]

	//verify by great cities
	var greatCities = anpEntities.cities.filter(function(c) { return c.name.toLowerCase().indexOf('sao paulo') > -1 })

	greatCities.push(anpEntities.cities.filter(function(c) { return c.name.toLowerCase().indexOf('curitiba') > -1 })[0])

	greatCities.push(anpEntities.cities.filter(function(c) { return c.name.toLowerCase().indexOf('brasilia') > -1 })[0])
...
module.updateRegions = function(cb) {
	...
	async.parallel([
		...
		if(err) cb(err)
			...
			fs.writeFileSync(path.resolve(entitiesPath, 'fuelTypes.json'), JSON.stringify(anpEntities.fuelTypes))
			fs.writeFileSync(path.resolve(entitiesPath, 'states.json'), JSON.stringify(anpEntities.states))
			fs.writeFileSync(path.resolve(entitiesPath, 'cities.json'), JSON.stringify(anpEntities.cities))
...
module.updateAllStations = function(cb, err) {
	...
	if(err) cb(err)
		...
		module.getCaptcha(anpEntities.cities[0], new Date(), function(err, res) {
			//create heap of functions that will get stations info
			anpEntities.cities.forEach(function(city) {
				anpEntities.fuelTypes.forEach(function(fuelType) {
			...
			async.parallelLimit(stationsFunctions, 5, function (error, response) {
				...
				fs.writeFileSync(path.resolve(entitiesPath, 'sellPrices.json'), JSON.stringify(anpEntities.sellPrices))
				fs.writeFileSync(path.resolve(entitiesPath, 'stations.json'), JSON.stringify(anpEntities.stations))
		...
		function(err, res) {
			if (err) cb(err)
				...
				$('select[name=selEstado]').find('option').each(function(i, el) {
					anpEntities.states.push(strOp.strToAnpEntity($(el).val()))
				})
				$('select[name=selCombustivel]').find('option').each(function(i, el) {
					anpEntities.fuelTypes.push(strOp.strToAnpEntity($(el).val()))
				})
		...
		function(err, res) {
			if (err) cb(err)
				...
				$('select[name=selMunicipio]').find('option').each(function(i, el) {
					anpEntities.cities.push(strOp.strToAnpEntity($(el).val()))
				})
		...
		function (err, res) {
			if(err) cb(err)
				...
				$('select[name=BAIRRO]').find('option').each(function(i, el) {
					...
					anpEntities.neighborhoods.push($(el).val().replace(/\@/g, ' '))
				})

				console.log('completed get neighborhoods')

				cb(anpEntities.neighborhoods)
		}
	)
}
...
module.getStationsByCity = function(city, fuelType, value, cb) {
	...
	var scratchStations = function(err, res) {
		if (err) cb(err)
			...
			anpEntities.stations = anpEntities.stations.concat(stationsFN.concat(stationsNFN))
			if(sellPrice.hasOwnProperty('media')) {
				anpEntities.sellPrices.push(sellPrice)
			}
