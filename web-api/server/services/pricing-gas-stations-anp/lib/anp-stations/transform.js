...
module.exports.postProcessStations = function(cb) {
	...
	anpEntities.entities.cities.forEach(function(city) {
		city.stations = anpEntities.entities.stations.filter(function(s) {
...
module.formatStationObj = function(station) {
	var fuelType = anpEntities.entities.fuelTypes.filter(function(f) {
		return f.id == station.fuelTypeId
	})[0]
	...
	var resolveConflicted = function(a, b, i) {
		if (a[i] && b[i] && a[i] != b[i]) {
			...
			if (!b.conflicted.hasOwnProperty(i)) {
				...
				var hasAValue = b.conflicted[i].some(function(o) {
					return o == a[i]
				})
				var hasBValue = b.conflicted[i].some(function(o) {
					return o == b[i]
				})
				if (!hasAValue) {
